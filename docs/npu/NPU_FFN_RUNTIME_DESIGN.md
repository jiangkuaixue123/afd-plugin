...
NPU Worker / ModelRunner 的开发和验证直接面向 `camp2pconnector`。FFN daemon
loop 应通过真实 connector 接收 Attention 侧 metadata，并完成真实 A2E/E2A 闭环。
...
Phase 1 应只支持 `camp2pconnector` 和单流通信：
...
4. 实现 `AFDNPUFFNModelRunner(NPUModelRunner)`，先支持 eager 单流
   `execute_ffn_step()`。
5. 接入 `camp2pconnector` 的 FFN 侧方法，至少覆盖 `recv_dp_metadata_list()`、
   `recv_attn_output()`、`send_ffn_output()` 和关闭逻辑。
6. 用 `camp2pconnector` 跑通 NPU FFN daemon loop，验证
   `recv -> compute_ffn_output -> send` 的通信闭环。
7. 完善 `camp2pconnector` 的 `create_recv_metadata()`、`recv_attn_output()`、
