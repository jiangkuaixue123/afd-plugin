...
| 4 | connector 名称 | 已注册 `p2pconnector`、开发调试用 `npudummyconnector` 和 placeholder `camp2pconnector` | 当前只看 NPU `camp2pconnector` | **已决策**：生产迁移只分析和迁移 `camp2pconnector`；`npudummyconnector` 只用于开发调试和 runtime smoke |
...
- **配置与注册**：`AFDConfig` 已允许 `npudummyconnector` 和 `camp2pconnector`；
  `AFDConnectorFactory` 已静态注册 `npudummyconnector`、`camp2pconnector`；
...
- `camp2pconnector` 目前只是 import-safe placeholder，真实 HCCL/CAM/A2E/E2A 通信尚未
  接入。
- Ascend custom op 隔离目标还没有完全落地：当前 C++ binding / 测试仍注册和调用
  `torch.ops._C_ascend.*`，loader 仍查找
  `_cann_ops_custom/vendors/vllm-ascend`；目标应切到 `torch.ops.afd_ascend.*` 和
  `vendors/afd-plugin`。
...
4. **NPU Phase 3A：Ascend custom op 隔离，当前优先级最高**
   在真实 `camp2pconnector` 接入前，先把现有 A2E/E2A extension 从临时兼容形态收敛到
   插件隔离形态：package-owned extension 保持 `afd_plugin._C_ascend`；dispatcher
   namespace 改为 `torch.ops.afd_ascend.a2e/e2a`；CANN vendor 目录改为
   `afd_plugin/_cann_ops_custom/vendors/afd-plugin/...`；加载 `libcust_opapi.so` 时使用
   明确包内路径；增加 vLLM-Ascend 先加载、AFD 后加载的 coexistence smoke。
