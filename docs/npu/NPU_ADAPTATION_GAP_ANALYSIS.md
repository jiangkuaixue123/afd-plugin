...
- **NPU dummy connector 已决策**：Phase 1 先实现一个仅用于开发调试的
  `npudummyconnector`，用于在 NPU 上跑通 Attention/FFN worker、model runner、
  class path、forward context 和 DP metadata 通道。它不属于生产迁移目标，稳定后再接
  `camp2pconnector`。
...
| 4 | connector 名称 | 只注册 `p2pconnector` | 当前只看 NPU `camp2pconnector` | **已决策**：生产迁移只分析和迁移 `camp2pconnector`；Phase 1 允许新增仅用于开发调试的 `npudummyconnector` |
...
Phase 1 先新增 `npudummyconnector` 作为 NPU runtime 开发脚手架。它只用于验证：
...
`npudummyconnector` 不依赖 HCCL、`_C_ascend.a2e/e2a` 或 CAM op，不验证真实跨卡通信
性能。它不改变生产 connector 范围；`camp2pconnector` 仍是后续真实 connector 目标。
...
2. **NPU Phase 1：NPU runtime dummy 闭环**
   扩展 `AFDConfig`，允许 NPU runtime 使用 `npudummyconnector`；新增
   NPU Attention/FFN worker/model runner class path 和一个能在 NPU 环境启动的
   dummy connector。目标是先跑通 import/classpath、role validation、forward context、
   DP metadata、FFN daemon loop 和本地 passthrough/最小 FFN step，不要求真实 A2E/E2A
   通信。

3. **NPU Phase 2：connector contract 归一**
   引入 `AFDRecvOutput`、`connector_data`、metadata 构造/更新和 DP metadata 收发等
   接口，先让 GPU connector 和 `npudummyconnector` 适配新 contract，再让 NPU runtime
   接入统一 contract。`compute_moe` / `select_experts` 当前不进入统一 contract。

4. **NPU Phase 3：Ascend connector 接入**
   以 CPU-safe import 为前提，将 `camp2pconnector` 迁入插件，并沿用
   `AFDConnectorFactory.register_connector(...)` 做插件内静态注册。缺少 torch-npu
   或 custom op 时干净 skip。

5. **NPU Phase 4：模型/MoE 语义**
   迁移不依赖 `compute_gate_on_attention` 的 Ascend fused MoE 和 token dispatcher
   相关逻辑；`compute_gate_on_attention` 和权重加载裁剪当前明确不支持，后续单独
   设计。

6. **NPU Phase 5：ACL graph、ubatching 和性能**
