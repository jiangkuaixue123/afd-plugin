...
- **NPU dummy connector 已移除**：NPU 迁移不再保留开发调试用 dummy
  connector；runtime 验证直接面向真实 `camp2pconnector`。
...
| 4 | connector 名称 | 已注册 `p2pconnector` 和 `camp2pconnector` | 当前只看 NPU `camp2pconnector` | **已决策**：NPU 路径只分析、迁移和验证 `camp2pconnector` |
...
Phase 1 不再引入 NPU dummy connector。runtime 验证直接使用 `camp2pconnector`，
覆盖 NPU Attention/FFN worker class path、metadata 构造与发送、FFN daemon loop、
forward context 注入和真实 A2E/E2A 通信闭环。
...
- **配置与注册**：`AFDConfig` 已允许 `camp2pconnector`；
  `AFDConnectorFactory` 已静态注册 `camp2pconnector`；
...
2. **NPU Phase 1：NPU runtime 控制面，已完成开发**
   已实现 NPU Attention/FFN worker/model runner class path、role validation、
   forward context metadata mirror、DP metadata 通道、FFN daemon loop 和
   `camp2pconnector` 控制面接口。真实 Ascend 环境的
