...
可行方向是：用 AFD additional config 表达 async-DP 调度语义，在插件内定义统一
判定 helper，例如 `is_afd_async_dp(vllm_config)`，再用少量 AFD-scoped compat patch
承接 vLLM engine/core-client 的调度差异。不要把方案建立在直接修改 vLLM
`ParallelConfig` dataclass、新增 vLLM CLI 参数、或动态给
`parallel_config` 补 `async_dp` 属性上。

推荐按核心 / 非核心拆分处理：

- 第一批最小可行链路：vLLM 配置判定、engine proc 选择、coordinator wave 关闭、
  client `FIRST_REQ` 跳过、forward context 中 DP metadata 创建跳过、NPU runner
  DP sync 跳过。
...
- 也不动态给 `parallel_config` 补 `async_dp` 兼容属性。
- 在 plugin-owned `additional_config["afd"]` 中新增公开 JSON key
  `"async": true`；Python dataclass 字段名使用 `async_dp`，因为 `async`
  是 Python 关键字。
- `async=true` 必须搭配 `connector="afdasyncconnector"`；反之，如果选择
  `afdasyncconnector` 但未开启 `async=true`，NPU runtime 校验应 fail fast。
- 在插件内新增统一 helper，例如：
...
def is_afd_async_dp(vllm_config: object) -> bool:
    ...
    return (
        afd_config.enabled
        and afd_config.async_dp
        and afd_config.connector == "afdasyncconnector"
    )
```

- 对插件已有代码中直接读取 `parallel_config.async_dp` 的位置，改为 helper 或
  connector capability；不要继续兼容 `parallel_config.async_dp` 字段。
...
- 需要把其中对 `parallel_config.async_dp` 的硬依赖改为插件 helper。
...
- patch `vllm.v1.engine.utils.launch_core_engines()`，在有完整 `vllm_config` 的
  入口判断 `is_afd_async_dp(vllm_config)`，并仅在该调用期间包装
  `DPCoordinator` 创建逻辑，强制 `enable_wave_coordination=False`。
- 不 patch 全局 `DPCoordinator(parallel_config, ...)` 行为，因为该构造器只有
  `parallel_config`，没有 plugin-owned AFD config；也不为了它给
  `parallel_config` 补 `async_dp` 属性。
...
- 第一版不 patch standalone `coordinate_batch_across_dp(parallel_config, ...)`。
- 原因：该函数只有 `parallel_config`，没有 `vllm_config` 或 AFD config；继续
  patch 它会迫使插件维护 `parallel_config.async_dp` 兼容属性或额外全局注册表。
- 通过 patch `set_forward_context(vllm_config, ...)` 避免该路径为 AFD async
  创建 `DPMetadata` 和触发原生 DP coordination。
- plugin-owned NPU runner 内部的 DP sync / metadata control-plane 分支继续使用
  AFD config helper 或 connector capability。
- 如果后续 smoke 发现仍有必须处理的直接调用点，优先 patch 具有 `vllm_config`
  的调用方，而不是 patch 只有 `parallel_config` 的底层函数。
...
- 把分支判断统一到 `is_afd_async_dp()` 或 connector capability，不直接依赖
  `parallel_config.async_dp`，也不动态补该属性。
...
- `afd_plugin.compat.async_dp`
  ...
  - `is_afd_async_attention_dp(vllm_config)`
  - AFD async 相关公共判定，不放在 hot path 里做复杂反射，不写
...
- `afd_plugin.compat.patches.async_dp_forward_context`
  - `set_forward_context()` patch。
  - 覆盖改动点 6；第 7 点不 patch standalone 底层函数，除非后续 smoke 证明必须。
...
1. 在 `AFDConfig` 中新增 `async_dp` 字段，并支持 JSON key `"async"`。
2. 新增 `is_afd_async_dp()` / `is_afd_async_attention_dp()` helper。
3. 调整插件内所有直接读 `parallel_config.async_dp` 的位置，不再兼容该字段。
4. 扩展 engine core patch，让 AFD async Attention 使用普通 `EngineCoreProc`。
5. Patch coordinator wave coordination 和 client `FIRST_REQ`。
6. Patch forward context，跳过 DP metadata 创建。
7. 跑聚焦测试或 NPU smoke，验证 patch 幂等和非 AFD 行为不变。
8. 再做 NPU smoke，确认 `afdasyncconnector` + DeepSeek 路径不会进入 native
   Ascend FusedMoE；如果会，优先修正插件接管路径，不迁移第 10 点 stub。
9. 根据 profile / 性能验证需要决定是否迁移 force load balance、layer sharding、
   profiler schedule。
