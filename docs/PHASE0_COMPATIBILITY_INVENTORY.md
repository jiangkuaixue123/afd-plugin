...
in-tree AFD 改动逐项归类，判断在 vLLM `v0.20.2` 上哪些可以通过插件机制迁出，
...
| Compat Shim | 小范围兼容层，适配 vLLM `v0.20.2` 的接口缺口 |
...
- 不假设 vLLM `v0.20.2` 有直接传入 model runner class 的公开参数。
...
5. `GPUFFNModelRunner` 中依赖的 vLLM import 是否都存在于 `v0.20.2`，哪些需要
   迁移到 `compat`。
